...
public class Localization_SOURCE_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
            ...
            if(pb($"Apply To All Keys in '{categories[l.selectedCategory]}' Category"))
            {
                foreach(Localization_SOURCE._LocalizationSelector sel in l.LocalizationSelector)
                {
                    ...
                    if (l.quickActions.SpecificTextMeshes.Length > 0)
                        sel.AT_TextMeshbject = l.quickActions.SpecificTextMeshes;
                }
            }
        }
        pve();
        ...
        if (l.LocalizationSelector.Count > 0) pDrawList();
        pve();
        pve();
    }

    private void pDrawList()
    {
        for (int i = 0; i < l.LocalizationSelector.Count; i++)
        {
            ...
            switch(sec.AssignationType)
            {
                ...
                case Localization_SOURCE._LocalizationSelector._AssignationType.LocalizationComponent:
                    pv();
                ...
                case Localization_SOURCE._LocalizationSelector._AssignationType.SpecificTextMesh:
                    pP(item.FindPropertyRelative("AT_TextMeshbject"), "Specific Text Mesh", "Assign specific Text Mesh objects", true);
                    break;
            }
